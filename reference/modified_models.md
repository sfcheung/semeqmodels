...
- parallel:
  ...
  Whether parallel processing will be used when fitting the models. To
  be passed to
  [`modelbpp::fit_many()`](https://sfcheung.github.io/modelbpp/reference/fit_many.html).

- ncores:

  The number of CPU cores to be used if `parallel` is `TRUE`. To be
  passed to
  [`modelbpp::fit_many()`](https://sfcheung.github.io/modelbpp/reference/fit_many.html).
...
- remove_dropped:
  ...
  path. For internal use. Should not be changed.

- remove_zeros:

  Whether a parameter explicitly fixed to `zero` will be removed before
  generating modified models. For internal use. Should not be changed.
...
one or more paths added (set to free).
