...
to the dataset `data_test_3obvs`
...
NOTE: For now, `semeqmodels` only supports
...
model *df* equals zero. Therefore,
...
`partables_plots()` can be used. It uses
...
The model labeled `Original` is the
...
- Paths or covariances different from
...
the help page for `plot.partables_plots()`.
