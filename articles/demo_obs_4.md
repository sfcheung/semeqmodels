...
A parallel mediation model will be used as an example to illustrate how
to use `semeqmodels`.

## Original Model

Suppose this is the original model we fitted to the dataset
`data_test_4obvs` (installed with the package):

![Parallel Mediation Model](demo_obs_4_model-1.png)
...
NOTE: For now, `semeqmodels` only supports models fitted with
...
The argument `curve_cov_settings`, with `list(base_curve = 4)`, is used
to make the curves for covariances easier to read.
...
![Empirically Equivalent Models](demo_obs_4_eqmodels-1.png)
...
![fx Must Not Be a y-Variable](demo_obs_4_not_y-1.png)
...
Suppose that we have theoretical reasons to argue that `fm1` and `fm2`
must be affected directly by `fx`, and so would like to keep models with
these two paths.

We can use
[`have_pars_all()`](https://sfcheung.github.io/semeqmodels/reference/partable_select.md)
to specify paths that must be present in a model
...
![Some Paths Must Be Present](demo_obs_4_have_pars_all-1.png)

Some Paths Must Be Present

### Must Not Have Any Paths from `fm1` to `fm2`

Suppose that, theoretically, `fm1` cannot have any effect on `fm2`,
...
![No Paths from fm1 to fm2](demo_obs_4_no_paths-1.png)
...
![Chained Filter](demo_obs_4_chained-1.png)
